...
check results compatible with the ecom-analytics scoring system.
...
_CTA_SELECTORS = [
    ...
    "button:has-text('Shop Now')", "button:has-text('Order Now')",
    ...
    "button:has-text('Contact')", "button:has-text('Subscribe')",
    ".cta", "[class*='cta']",
    # Links with CTA-like paths
    "a[href*='buy']", "a[href*='demo']", "a[href*='trial']",
    "a[href*='contact']",
]
...
_CHAT_SELECTORS = [
    ...
    "[class*='tidio']", "[class*='crisp']", "[class*='tawk']",
]
...
def analyze_page(
    ...
    try:
        with sync_playwright() as p:
            ...
            data.has_reviews = any(
                k in page_text for k in ["review", "rating", "stars", "★"]
            )
...
def build_site_checks(pages: list[PageAuditData]) -> list[CheckResult]:
    ...
    if not valid:
        ...
        checks.append(CheckResult(
            check_id="SA01", category="site", severity="critical",
            result="fail",
    ...
    checks.append(CheckResult(
        ...
        current_value=any_cta_desktop, threshold=True,
        recommended_action="" if any_cta_desktop else "Add a prominent CTA (Buy Now, Add to Cart) in the first viewport.",
    ))
    ...
    checks.append(CheckResult(
        check_id="SA02", category="site", severity="high",
        result=sa02_result, message=sa02_msg,
        current_value=best_area, threshold=3000,
        recommended_action="" if sa02_result == "pass" else "Increase CTA button size and use high-contrast colors.",
    ))

    # SA03 — Mobile CTA visibility
    any_cta_mobile = any(p.cta_above_fold_mobile for p in valid)
    checks.append(CheckResult(
        check_id="SA03", category="site", severity="critical",
        ...
        current_value=any_cta_mobile, threshold=True,
    ...
    if form_pages:
        ...
        checks.append(CheckResult(
            check_id="SA04", category="site", severity="high",
            result=sa04_result, message=sa04_msg,
            current_value=worst_fields, threshold=5,
            recommended_action="" if sa04_result == "pass" else "Reduce form fields to 5 or fewer; use progressive disclosure.",
        ))
    ...
    checks.append(CheckResult(
        check_id="SA05", category="site", severity="critical",
        result="pass" if sa05_pass else "fail",
        message="Mobile responsive: viewport meta present, no horizontal scroll" if sa05_pass
        else f"Mobile issues: viewport_meta={'yes' if all_viewport else 'MISSING'}, h_scroll={'yes' if any_hscroll else 'no'}",
        current_value=sa05_pass, threshold=True,
    ...
    if lcp_values:
        ...
        checks.append(CheckResult(
            check_id="SA06", category="site", severity="high",
            result=sa06_result, message=sa06_msg,
            current_value=worst_lcp, threshold=2500,
            recommended_action="" if sa06_result == "pass" else "Optimize images, reduce JS, use CDN to improve LCP.",
        ))
    ...
    if cls_values:
        ...
        checks.append(CheckResult(
            check_id="SA07", category="site", severity="medium",
            result=sa07_result, message=sa07_msg,
            current_value=worst_cls, threshold=0.1,
            recommended_action="" if sa07_result == "pass" else "Set explicit dimensions on images/ads to prevent layout shifts.",
        ))
    ...
    checks.append(CheckResult(
        check_id="SA08", category="site", severity="high",
        result=sa08_result, message=sa08_msg,
        current_value=best_trust, threshold=2,
        recommended_action="" if sa08_result == "pass" else "Add customer reviews, trust badges, or satisfaction guarantees.",
    ))
    ...
    checks.append(CheckResult(
        check_id="SA09", category="site", severity="medium",
    ...
    checks.append(CheckResult(
        check_id="SA10", category="site", severity="medium",
        ...
        current_value=has_contact, threshold=True,
    ...
    checks.append(CheckResult(
        check_id="SA12", category="site", severity="medium",
        result=sa12_result, message=sa12_msg,
        current_value=best_img, threshold="2-20",
    ...
    checks.append(CheckResult(
        check_id="SA13", category="site", severity="medium",
        result=sa13_result, message=sa13_msg,
        current_value=all_https, threshold=True,
        recommended_action="" if sa13_result == "pass" else "Ensure HTTPS and add payment badges / privacy policy link.",
    ))
    ...
    checks.append(CheckResult(
        check_id="SA14", category="site", severity="medium",
        result=sa14_result, message=sa14_msg,
        current_value=all_nav, threshold=True,
        recommended_action="" if sa14_result == "pass" else "Add consistent navigation with logo linking to homepage.",
    ))
    ...
    if font_sizes:
        ...
        checks.append(CheckResult(
            check_id="SA15", category="site", severity="low",
            result=sa15_result, message=sa15_msg,
            current_value=min_font, threshold=16,
            recommended_action="" if sa15_result == "pass" else "Increase base font size to at least 16px for mobile readability.",
        ))
