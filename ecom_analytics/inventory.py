...
def overstock_analysis(
    ...
    if "sku" in orders.columns:
        ...
        sku_qty = orders.groupby("sku")["quantity"].sum() if "quantity" in orders.columns else orders.groupby("sku").size()
...
def safety_stock_calculation(
    orders: pd.DataFrame, service_level: float = 0.95
) -> pd.DataFrame:
    """Calculate safety stock per SKU.

    Uses: safety_stock = Z × σ_demand × √(lead_time).
    Default lead time = 14 days if not provided.
    """
    ...
    orders = orders.copy()
    orders["order_date_day"] = orders["order_date"].dt.date
    qty_col = "quantity" if "quantity" in orders.columns else "amount"

    daily = orders.groupby(["sku", "order_date_day"])[qty_col].sum().reset_index()
    stats = daily.groupby("sku")[qty_col].agg(["mean", "std"]).fillna(0)
    stats.columns = ["daily_mean", "daily_std"]

    lead_time = 14  # days default
    stats["safety_stock"] = z * stats["daily_std"] * np.sqrt(lead_time)
    stats["reorder_point"] = stats["daily_mean"] * lead_time + stats["safety_stock"]
