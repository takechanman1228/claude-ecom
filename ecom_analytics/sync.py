...
def sync_shopify(
    ...
    until: str | None = None,
    mode: str = "full",
    out_dir: str | Path = ".ecom-analytics/data",
    ...
    if mode == "incremental":
        ...
        if existing_orders is not None:
            orders_norm = pd.concat([existing_orders, orders_norm]).drop_duplicates(
...
def load_synced_data(
    ...
    if orders is None:
        raise FileNotFoundError(
            f"No synced order data found in {out_path}. "
            "Run 'ecom-analytics shopify sync' first."
        )
    if items is None:
    ...
    if products is not None:
        products_compat = products.rename(columns={
            "title": "name",
        })
