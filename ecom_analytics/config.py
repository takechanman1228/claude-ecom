...
def load_config(path: str | Path | None = None) -> ShopifyConfig:
    ...
    2. Local: ``<cwd>/.ecom-analytics/config.toml``
    3. Global: ``~/.ecom-analytics/config.toml``
    ...
    if config_path is None:
        ...
        raise FileNotFoundError(
            ...
            "Run 'ecom-analytics shopify setup' to create one."
        )
    ...
    if not token:
        raise ValueError(
            "No access token found in config or SHOPIFY_ACCESS_TOKEN env var."
        )
...
def save_config(
    ...
    *,
    ...
    global_ : bool
        If *True* and *path* is None, write to ``~/.ecom-analytics/config.toml``.
...
def _ensure_gitignore(project_root: Path) -> None:
    """Add ``.ecom-analytics/`` to the nearest ``.gitignore`` if not present."""
    gitignore = project_root / ".gitignore"
    entry = ".ecom-analytics/"
