...
from ecom_analytics.shopify_api import build_parent_child_map
...
def normalize_products(rows: list[dict]) -> pd.DataFrame:
    ...
    for gid, product in parents.items():
        ...
        for variant in product.get("_children", []):
            ...
            records.append({
                ...
                "tags": ",".join(product.get("tags", [])) if isinstance(product.get("tags"), list) else str(product.get("tags", "")),
            })
    ...
    if len(df) == 0:
        return pd.DataFrame(columns=[
            ...
            "price", "compare_at", "cost", "vendor", "tags",
        ])
    return df
...
def build_orders_compat(
    ...
    order_cols = orders_norm[["order_id", "order_date", "customer_id",
                              "discount_amount", "shipping_amount"]].copy()
