...
class ShopifyClient:
    ...
    def __init__(self, config: ShopifyConfig, timeout: float = 30.0):
        if httpx is None:
            raise ImportError("httpx is required for Shopify API access. Install with: pip install claude-ecom[api]")
        self._config = config
...
class BulkRunner:
    ...
    def _submit(self, query: str) -> str:
        ...
        result = self._client.graphql(BULK_SUBMIT_MUTATION, variables={"query": query})

        op_data = result["data"]["bulkOperationRunQuery"]
        errors = op_data.get("userErrors", [])
        if errors:
            raise ShopifyAPIError(f"Bulk operation submit errors: {json.dumps(errors)}")
    ...
    def _poll_until_done(
        ...
        while time.time() < deadline:
            ...
            if op is None:
                raise ShopifyAPIError("No current bulk operation found. It may have been cancelled.")
            ...
            if status == "COMPLETED":
                ...
                self._clear_running_op()
                raise ShopifyAPIError(f"Bulk operation {status}: error={op.get('errorCode')}")

            logger.debug("Polling: status=%s, objects=%d", status, obj_count)
            time.sleep(interval)

        raise ShopifyAPIError(f"Bulk operation timed out after {self._timeout_minutes} minutes")
